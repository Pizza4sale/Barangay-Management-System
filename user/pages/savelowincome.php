...
$address = $_POST['street']." Barangay Milawid, Panukulan, Quezon";
   ...
   $qry = "INSERT INTO `tbl_lowincome`(`reqnum`, `fname`, `mname`, `lname`, `age`, `marital`, `address`, `status`, `pay_status`, `datetimereq`, `fromuser`,phonenumber,work,monthly_income,workadd,bday,uploadreq,photoupload,datereleased,captain,old_ctrl,date_rel) VALUES ('".$new."' ,'".$_POST['fname']."','".$_POST['mname']."','".$_POST['lname']."','".$age."','".$_POST['civil']."','".$address."','0','NEW','".$formated_DATETIME."', '".$uname."','".$_POST['number']."','".$_POST['work']."','".$_POST['monthly']."','".$_POST['workadd']."','".$_POST['bday']."','".$filename1."','N/A','','','','')";
   $updd = $conn->query($qry);


   $expire=date('Y-m-d H:i:s', strtotime($formated_DATETIME. '+2 days'));
   $sa = "INSERT INTO `tbl_request`(`type`, `cnum`, `datesent`, `status`,request_type,expiredate,fromuser,oldcnum,datedeleted) VALUES('lowincome','".$new."','".$formated_DATETIME."','Active','PENDING','".$expire."','".$uname."','','')";
      $conn->query($sa);

   $upd_incre = "UPDATE `tbl_increments` SET num = num + 1 WHERE type= 'request' ";
   $updd = $conn->query($upd_incre);
   // header("Location:service_number_lowincome.php?idres={$new}");
...
//   );
  ...
  // $result = adyen($full_name, 'lowincome', '/user/pages/service_number_lowincome.php', $new);

  // header('Location: ' . $result['action']['url']);
  header('Location: service_number_lowincome.php?cnum=' . $cnum);
    ?>
