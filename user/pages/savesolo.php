...
$address = $_POST['street']." Barangay Milawid, Panukulan, Quezon";
   ...
   $qry = "INSERT INTO `tbl_soloparent`(`reqnum`, `fname`, `mname`, `lname`, `age`, `marital`, `address`, `status`, `pay_status`, `datetimereq`, `fromuser`,phonenumber,work,workadd,child,bday,uploadreq,photoupload,datereleased, captain, old_ctrl,date_rel) VALUES ('".$new."' ,'".$_POST['fname']."','".$_POST['mname']."','".$_POST['lname']."','".$age."','".$_POST['civil']."','".$address."','NEW','NEW','".$formated_DATETIME."', '".$uname."','".$_POST['number']."','".$_POST['work']."','".$_POST['workadd']."','".$_POST['child']."','".$_POST['bday']."','".$filename1."','N/A', '', '', '', '')";

   $updd = $conn->query($qry);


   $expire=date('Y-m-d H:i:s', strtotime($formated_DATETIME. '+2 days'));
   $sa = "INSERT INTO `tbl_request`(`type`, `cnum`, `datesent`, `status`,request_type,expiredate,fromuser,oldcnum,datedeleted) VALUES('soloparent','".$new."','".$formated_DATETIME."','Active','PENDING','".$expire."','".$uname."','','')";
      $conn->query($sa);

   $upd_incre = "UPDATE `tbl_increments` SET num = num + 1 WHERE type= 'request' ";
   $updd = $conn->query($upd_incre);
  //  header("Location:service_number_soloparent.php?idres={$new}");
...
//   );
  ...
  // $result = adyen($full_name, 'soloparent', '/user/pages/service_number_soloparent.php', $new);

  // header('Location: ' . $result['action']['url']);
  header('Location: service_number_soloparent.php?cnum=' . $cnum);
?>
