...
$query = "SELECT * FROM tbl_indigency where reqnum= '".$ctrl."' ";  
...
while($row = mysqli_fetch_array($result2))  
 {  
   ...
   if($row["captain"] =='okay' && $row["datereleased"]==""){
    $datee = date('Y-m-d');
    $upd="UPDATE `tbl_indigency` SET datereleased ='".$formated_DATETIME."', date_rel='".$datee."' where reqnum='".$ctrl."' ";
...
class PDF extends TCPDF
{
        public function Header(){
          ...
          //  $this->Image($imageFile,150,5,25,'','JPG','','T',false,300,'',false,false,0,false,false,false);
           ...
           $this->Cell(180,5,'________________________________________________________________________________',0,1,'C');

             $this->Ln(6);
           $this->SetFont('times','B',12, '', true);
           $this->Cell(180,5,'*CERTIFICATE OF INDIGENCY*',0,1,'C');
...
$pdf->Ln(10);
$pdf->Cell(8,5,'',0,0,'L',0);
$pdf->MultiCell(160,5,"         This is to CERTIFY that ". strtoupper($requestor_name)." , ".$age." years old, ".$civil." , Filipino and resident of Barangay Milawid, Panukulan, Quezon. This further certify that the above name of the person has good moral and law-abiding citizen but unfortunately, he/she belong to the poor family in our community.",0,'J',0,0,'','',true);
...
$pdf->Ln(10);
$pdf->Cell(10,5,'',0,0,'L',0);
$pdf->MultiCell(180,5,"         Issued this ".$formated_DATETIME." day of ".$formated_monthyer." at Barangay Milawid, Panukulan, Quezon." ,0,'L',0,0,'','',true);
...
while($rowx = mysqli_fetch_array($result2a))  
{
    $pdf->Image($imageFile3, 140,168, 40, 20,'JPG','','',false,200,'',false,false,0,false,false,false);
    $pdf->Cell(35,5,strtoupper($rowx['org_name']),0,0,'C',0);
}
$pdf->Ln(6);
$pdf->Cell(20,5,'',0,0,'L',0);
