...
<center>
   ...
   <?php 
   $style1 = "style=''";
        $sqlrt01 = "SELECT * FROM `tbl_lowincome` where reqnum ='".$_GET['cnum']."' ";
        $processQueryrt01 = $conn->query($sqlrt01);
        while ($resultQueryrt01 = $processQueryrt01->fetch_assoc())
        {  

         ?> 
        <form method="post" action="savelowincome.php"  enctype="multipart/form-data" >
        <br>
         <div class="card" >
          <h2  style="padding-left:-10px;font-size:20px;background-color: grey;" class="badge rounded-pill"> &nbsp Certificate of Low Income - Control No.: <?php echo $_GET['cnum']; ?> &nbsp </h2>
<center>
            ...
            <div class="card-body" style="background-color:#25304E;width:90%;border-radius:12px" >
              ...
              <div class="row mb-3 mt-1" style="padding-left : 30px;padding-right : 30px;padding-top: 1px;">
                ...
                <div class="col-lg-4">
                 <label><small class="text-muted">MONTHLY INCOME</small></label >
                  <input type="text" class="form-control" name="monthly"  value="<?php echo $resultQueryrt01['monthly_income']; ?>" autocomplete="off" <?php echo $style1; ?>>
                </div>
                </div>
               ...
               <?php if($resultQueryrt01['pay_status']=="NEW"){ ?>
                <button onclick="return confirm('Are you sure you want to process this request ?')" type="submit" style="width: 20%;color:white;border-radius: 5px;" class="btn btn-primary" ><i class="fas fa-save"></i>  Process / Save</button>
                  <a href="payment_ref_edit.php?cnum=<?php echo $_GET['cnum']; ?>&oldcnum=A&from=lowincome" style="width: 20%;color:white;border-radius: 5px;" class="btn btn-danger" > <i class="fas fa-arrow-alt-circle-left"></i> Back to Home</a>
                <?php }else{?>
                    <br>
