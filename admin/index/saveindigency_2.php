...
$address = $_POST['street']." Barangay Milawid, Panukulan, Quezon";
   ...
   $qry = "INSERT INTO `tbl_indigency`(`reqnum`, `fname`, `mname`, `lname`, `age`, `marital`, `address`, `status`, `pay_status`, `datetimereq`,phonenumber,bday,datereleased) VALUES ('".$new."' ,'".$_POST['fname']."','".$_POST['mname']."','".$_POST['lname']."','".$age."','".$_POST['civil']."','".$address."','OLD','NEW','".$formated_DATETIME."', '".$_POST['number']."', '".$_POST['bday']."','".$formated_DATETIME."')";
   $updd = $conn->query($qry);
         

   $upd_incre = "UPDATE `tbl_increments` SET num = num + 1 WHERE type= 'indigency' ";
   $updd = $conn->query($upd_incre);
   header("Location:payment_ref.php?cnum={$new}&from=indigency");
