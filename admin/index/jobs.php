    ...
    >
      ...
      <div class="page-wrapper">
        ...
        <div class="container-fluid">
          ...
          <div class="tab">
            <button class="tablinks" onclick="openCity(event, 'ab')" style="width:30%" id="defaultOpen"><i class="fas fa-puzzle-piece"> </i> List of Jobs</button>
            <button class="tablinks" onclick="openCity(event, 'cd')" style="width:30%"><i class=" fas fa-suitcase"></i> Add New Jobs</button>
            ...
            <div class="card">
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table id="example" class="table display" style="font-size:13px;color:black;" >
                      <thead>
                        <tr>
                           <th <?php echo $style1; ?>><b>JOB TITLE</b></th>
                           <th <?php echo $style1; ?>><b>POSTED DATE</b></th>
                           <th <?php echo $style1; ?>><b>CLOSED DATE</b></th>
               ...
               <?php    
                     //Code for displayed user in a table
                      $query = "SELECT distinct * FROM tbl_job order by status DESC";   
                      $result = mysqli_query($conn, $query);  
                      while($row = mysqli_fetch_array($result)) { ?>
                      <tr>
                          <td style="width:250px"><?php echo $row["job_title"]; ?></td>
                          <td style="width:120px"><?php echo $row["posted_date"]; ?></td>
                          <td style="width:120px"><?php if($row['closed_date'] != ""){ echo $row['closed_date']; }else{ echo"No available date."; }?></td> 
                          <td style="width:100px" ><?php if($row['status'] =="OPEN"){ ?>
                         <a class="badge btn-success" style="font-size: 13px;width:100%"  href="edit.php?from=job&type=<?php echo"edit";?>&id=<?php echo $row['jobid'];?>"><i class="fas fa-edit"></i> Edit</a>

                      
                          <a  class="badge btn-danger" style="font-size: 13px;width:100%"  onclick="return confirm('Are you sure you want to delete this job?')" href="delete.php?type=<?php echo"delete";?>&id=<?php echo $row['jobid'];?>&type=removealljob&from=job"><i class="fas fa-trash"></i> Delete</a>
                         <?php } ?>
                         </td>
                         <td style="width:60px"><?php if($row['closed_date'] == ""){?>
                             <a  class="badge btn-warning" style="font-size: 13px;" onclick="return confirm('Are you sure you want to close this job?')"  href="delete.php?type=<?php echo"close";?>&id=<?php echo $row['jobid'];?>&from=job"><i class='fa fa-times'></i> Close Job</a>
                          <?php }else{ ?>
                         <a  class="badge btn-danger" style="font-size: 13px;"onclick="return confirm('Are you sure you want to re-open this job?')"  href="delete.php?type=<?php echo"reopen";?>&id=<?php echo $row['jobid'];?>&from=job"><i class='fa fa-check'></i> Re-open</a>
                            <?php } ?>
                           </td> 
                        </tr>
                    <?php } ?>
