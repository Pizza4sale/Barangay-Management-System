...
<center>
   ...
   $style1 = "style=''";
        ...
        $sqlrt01 = "SELECT * FROM `tbl_brgyclearance` where reqnum='".$_GET['cnum']."' ";
        $processQueryrt01 = $conn->query($sqlrt01);
        while ($resultQueryrt01 = $processQueryrt01->fetch_assoc())
        {
          if($resultQueryrt01['status']=="NEW"){
              $d= "action='resave-clearance.php' ";
          }else{
              $d="action='resave-clearance.php' ";
          }
        ...
        <form method="post" action="resave-clearance.php" >
         <br>
         <div class="card" style="background-color:#25304E;width:90%;border-radius:16px" >
           <h2  style="padding-left:-10px;font-size:20px;background-color: #0C4AF3;" class="badge rounded-pill"> &nbsp New Application / Barangay Clearance - Control No.: <b><?php echo $_GET['cnum']; ?></b> &nbsp </h2>
                <div class="card-body">
                  <input type="hidden" name="from" class="form-control" value="<?php echo $_GET['from']; ?>" required>
                    <input type="hidden" name="reqnum" class="form-control" value="<?php echo $_GET['cnum']; ?>" required>
                <?php if($_GET['from']="renewclearance"){?>
                   <input type="hidden" name="newcnum" class="form-control" value="<?php echo ""; ?>" required>
                 <?php }?>
                  ...
                  <div class="col-lg-4">
                     <label><small class="text-muted">Gender</small></label ><Br>
                      <?php if( $resultQueryrt01['gender'] == "Female" ){ ?>
                        <input type="radio" style="font-size: 15px;" value="Female" name="gender"checked> <b style='color:white'>Female</b> &nbsp &nbsp &nbsp
                        <input type="radio" style="font-size: 15px" value="Male" name="gender"> <b style='color:white'>Male</b>
                      <?php }else{ ?>
                         <input type="radio" style="font-size: 15px;" value="Female" name="gender"> <b style='color:white'>Female</b> &nbsp &nbsp &nbsp
                         <input type="radio" style="font-size: 15px;" value="Male" name="gender" checked> <b style='color:white'>Male</b>
                      <?php } ?>
                  </div>
                </div>
                <div class="row mb-3" style="padding-left : 30px;padding-right : 30px;">
                  ...
                  <div class="col-lg-4" hidden>
                   ...
                   <a class='badge btn-warning' href="../../user/pages/file/<?php echo $file; ?>" target="_blank"><?php echo substr($file,0,30); ?></a>
                  </div>
                 </div>
                 <div class="row mb-3" style="padding-left : 30px;padding-right : 30px;">
                    <div class="col-lg-8">
                 ...
                 <div class="row mb-3" style="padding-left : 150px;padding-right :150px;">
                  <div class="col-lg-12">
              ...
              <?php if($resultQueryrt01['status'] == "NEW"){ ?>
                &nbsp  &nbsp <button name="save" onclick="return confirm('Are you sure you want to process this request ?')"type="submit" style="width: 25%;color:white;border-radius: 10px;"  class="btn btn-primary"><i class="fas fa-save"></i> Process / Save</button>

             <?php }elseif($resultQueryrt01['status'] == "OLD"){ ?>
              
                 &nbsp
                 <button name="edit" onclick="return confirm('Are you sure you want to process this request ?')"type="submit" style="width: 25%;color:white;"  class="btn btn-primary"><i class="fas fa-save"></i> Re-Process/Save</button> &nbsp 

                  <a href="clearance_file.php" style="width:25%;color:white;" class="btn btn-danger"> <i class="fas fa-arrow-alt-circle-left"></i> Back to Home</a>
               <?php } ?>
              </div>
              </div>
       </form>
